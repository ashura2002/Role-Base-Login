...
export const getAllEmployeesAttendance = async(req,res,next) =>{
      try {
        ...
        res.status(200).json({message: 'Attendance of all users', employeesAttendance})
      } catch (error) {
        next(error)
      }
}
...
export const postAttendance = async(req, res, next) =>{
    ...
    try {
        ...
        res.status(201).json({message: 'Attendance Successfully Recorded!', employeePostedAttendance})
    } catch (error) {
        console.log(error)
    }
}